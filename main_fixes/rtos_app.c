...
static void vControlTask(void *pv)
{
	...
	float vbat = 0.0f;            /* 最近一次电池电压 (10Hz 刷新, Safety 对 0 跳过检测) */
	uint32_t batDiv = 0;          /* 电池测量分频计数 */
	uint8_t lowBatPrinted = 0;    /* 低压打印边沿锁存 */
	(void)pv;
	...
	for (;;) {
		...
		if (g_att_ready) {
			...
			/* 电池是慢变量: 10Hz 测一次即可.
			 * ADC 轮询是阻塞调用, 200Hz 每测既浪费 (~31µs×200/s),
			 * ADC 故障时更会把 5ms 控制节拍拖住 */
			if (++batDiv >= 20) {
				batDiv = 0;
