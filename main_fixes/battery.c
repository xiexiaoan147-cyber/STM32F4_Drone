...
#define VREF           3.3f
/* 校准系数: 实测满电 4.20V 若读出 4.05V, 改为 4.20/4.05≈1.037 (默认 1.0 不校正) */
#define BAT_CALIB      1.000f
...
float Battery_GetVoltage(void)
{
	...
	/* 多次采样取平均, 抑制噪声.
	 * 超时 2ms 仅为故障兜底 (正常单次转换 ~4µs 立即返回);
	 * 本函数为阻塞式, 调用方应以 ≤10Hz 调用 (见 rtos_app.c 分频) */
	for (i = 0; i < 8; i++) {
	...
	return (float)(raw / 8) / 4096.0f * VREF * DIVIDER_RATIO * BAT_CALIB;
}
